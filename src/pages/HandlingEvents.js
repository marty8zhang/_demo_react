...
export default class HandlingEvents extends React.Component {
  ...
  /*
   * Note: This isn't standardised JS syntax to declare a class method, even though it seems
   * Node (v12+) supports it.
  ...
  /*
   * Note: Because we'll need to use `bind()` to pass in the id, the actual event object has to
   * become the second argument. In other words,
   * `this.handleArgumentsPassingMethodTwo.bind(this, 2)` below will create an anonymous function
   * for us similar to:
            ...
            <Row>
              <Col>
                <p>
                  The passed in id is
                  {' '}
                  {this.stringifyId(passedInId)}
                  .
                </p>
                <p>
                  <Button onClick={(event) => this.handleArgumentsPassingMethodOne(event, 1)}>
                    Pass Arguments
                  </Button>
                </p>
                <p>
                  <Button
                    onClick={this.handleArgumentsPassingMethodTwo.bind(this, 2)} // eslint-disable-line
                  >
                    Pass Arguments
                  </Button>
                </p>
