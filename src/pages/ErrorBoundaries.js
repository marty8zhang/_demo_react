...
class FatalErrors extends React.Component {
  ...
  render () {
    ...
    return (
      <>
        ...
        <p>To see how error boundary works, try to uncomment some error throwing code in the lifecycle methods of `FatalErrors`.</p>
        <p><Form.Control type="text" name="test" value={this.state.name} onChange={this.handleFormFieldChange} placeholder="Test" /></p>
      </>
    )
  }
  ...
  componentDidUpdate (prevProps, prevState, snapshot) {
    ...
    if (prevState.renderCount === 0) { // Indicate that `this.setState()` in `componentDidMount()` triggered the current update.
      this.setState({
        // Note: `componentDidUpdate()` can only happen after the second `render()` call, so technically speaking
        // `render()` count in this method should be `2`.
        renderCount: this.state.renderCount + 1,
      })
    }

    // The `render()` count here should return `1`, since `this.setState()` above hasn't taken effect yet.
    // throw new Error(
